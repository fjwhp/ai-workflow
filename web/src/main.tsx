  ...
  return <div className={sidebarCollapsed?"app-shell sidebar-collapsed":"app-shell"}>
    <aside className="sidebar">
      ...
      <div className="queue-nav"><span>我的工作</span>
        <Queue label="待人工审批" count={queues.approvals.length} active={queueFilter==="approvals"} onClick={()=>selectQueue("approvals")}/><Queue label="AI 待启动" count={queues.ready.length} active={queueFilter==="ready"} onClick={()=>selectQueue("ready")}/><Queue label="AI 执行中" count={queues.running.length} active={queueFilter==="running"} onClick={()=>selectQueue("running")}/><Queue label="自动化待接管" count={queues.automation.length} active={queueFilter==="automation"} onClick={()=>selectQueue("automation")}/><Queue label="已阻塞" count={queues.blocked.length} active={queueFilter==="blocked"} onClick={()=>selectQueue("blocked")}/>
      </div>
...
export function Dashboard({items,queues,queueFilter,onQueue,onClearFilter,onOpen}:any){const filterLabels:any={approvals:"待人工审批",ready:"AI 待启动",running:"AI 执行中",automation:"自动化待接管",blocked:"已阻塞"};return <div className="content"><section className="stats">
  <Stat label="待人工审批" value={queues.approvals.length} tone="amber" icon={<ClipboardCheck/>} active={queueFilter==="approvals"} onClick={()=>onQueue("approvals")}/><Stat label="AI 待启动" value={queues.ready.length} tone="green" icon={<Bot/>} active={queueFilter==="ready"} onClick={()=>onQueue("ready")}/><Stat label="AI 执行中" value={queues.running.length} tone="blue" icon={<RefreshCw/>} active={queueFilter==="running"} onClick={()=>onQueue("running")}/><Stat label="自动化待接管" value={queues.automation.length} tone="blue" icon={<RefreshCw/>} active={queueFilter==="automation"} onClick={()=>onQueue("automation")}/><Stat label="已阻塞" value={queues.blocked.length} tone="red" icon={<AlertTriangle/>} active={queueFilter==="blocked"} onClick={()=>onQueue("blocked")}/>
