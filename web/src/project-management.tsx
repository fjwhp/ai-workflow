...
export function mergeProjectDetails(state: ProjectDetailsState, generation: number, projectId: string, resource: DetailResource, result: DetailResult): ProjectDetailsState {
  ...
  else { delete values[projectId]; projectErrors[resource] = result.reason; }
...
export function ProjectManagement() {
  ...
  const [details, setDetails] = useState<ProjectDetailsState>({ generation: 0, knowledge: {}, memory: {}, errors: {} }), [busyId, setBusyId] = useState(""), [error, setError] = useState("");
  const projectRequestGeneration = useRef(0), detailGeneration = useRef(0);
  ...
  return <div className="content project-management"><section className="section"><div className="section-head project-page-head"><div><h2>本地项目</h2><p>维护仓库验证、执行边界与项目知识</p></div><div className="project-head-actions"><button className="secondary" onClick={() => loadProjects().catch(e => setError(e.message))}><RefreshCw size={16}/>重新加载</button><button className="primary" onClick={() => { setValidateOnOpen(false); setEditor("new"); }}><Plus size={16}/>新建项目</button></div></div>
    ...
    <div className="project-list">{view.list.length ? view.list.map(project => <ProjectBand key={project.id} project={project} knowledge={details.knowledge[project.id]} memory={details.memory[project.id]} detailErrors={details.errors[project.id]} busy={busyId === project.id} onEdit={() => { setValidateOnOpen(false); setEditor(project); }} onValidate={() => { setValidateOnOpen(true); setEditor(project); }} onRebuild={() => rebuild(project)} onArchive={() => requestArchive(project)}/>) : <div className="empty"><FolderGit2 size={25}/><b>{filter === "active" ? "还没有使用中的项目" : "还没有归档项目"}</b><span>{filter === "active" ? "新建项目并验证本地 Git 仓库" : "归档项目会显示在这里"}</span></div>}</div>
  </section>{editor && <ProjectEditor project={editor === "new" ? null : editor} readOnly={editor !== "new" && editor.status === "archived"} validateOnOpen={validateOnOpen} onClose={() => setEditor(null)} onSaved={async () => { setEditor(null); await loadProjects(); }}/>} {archiveTarget && <ArchiveDialog project={archiveTarget} busy={busyId === archiveTarget.id} error={archiveError} onCancel={() => { if (!busyId) setArchiveTarget(null); }} onArchive={archive}/>}</div>;
}

function ProjectBand({ project, knowledge, memory, detailErrors, busy, onEdit, onValidate, onRebuild, onArchive }: any) {
