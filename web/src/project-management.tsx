...
export function ProjectManagement() {
  const [projects, setProjects] = useState<Project[]>([]), [filter, setFilter] = useState<ProjectStatus>("active"), [editor, setEditor] = useState<Project | "new" | null>(null);
  ...
  const archive = async (project: Project) => { if (!window.confirm(`归档项目“${project.name}”？\n\n归档后它将不再出现在新需求的项目选择中，已有交付历史和知识记录仍会保留。`)) return; setBusyId(project.id); setError(""); try { await post(`/projects/${project.id}/archive`, {}); await loadProjects(); } catch (e: any) { setError(e.message.includes("活动交付") ? `无法归档“${project.name}”：项目正在用于活动交付，请先完成或停止相关需求。` : e.message); } finally { setBusyId(""); } };
  return <div className="content project-management"><section className="section"><div className="section-head project-page-head"><div><h2>本地项目</h2><p>维护仓库验证、执行边界与项目知识</p></div><button className="primary" onClick={() => setEditor("new")}><Plus size={16}/>新建项目</button></div>
    ...
    <div className="project-list">{view.list.length ? view.list.map(project => <ProjectBand key={project.id} project={project} inspection={inspections[project.id]} knowledge={knowledge[project.id]} memory={memory[project.id]} busy={busyId === project.id} onEdit={() => setEditor(project)} onRebuild={() => rebuild(project)} onArchive={() => archive(project)}/>) : <div className="empty"><FolderGit2 size={25}/><b>{filter === "active" ? "还没有使用中的项目" : "还没有归档项目"}</b><span>{filter === "active" ? "新建项目并验证本地 Git 仓库" : "归档项目会显示在这里"}</span></div>}</div>
  </section>{editor && <ProjectEditor project={editor === "new" ? null : editor} readOnly={editor !== "new" && editor.status === "archived"} onClose={() => setEditor(null)} onSaved={async () => { setEditor(null); await loadProjects(); }}/>}</div>;
}
...
function ProjectEditor({ project, readOnly, onClose, onSaved }: { project: Project | null; readOnly: boolean; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  const [state, dispatch] = useReducer(submissionReducer<Form>, initialSubmissionState(source)), [validation, setValidation] = useState<Validation | null>(project && !readOnly ? { repoPath: project.repoPath, defaultBranch: project.defaultBranch, result: { valid: true, repoPath: project.repoPath, defaultBranch: project.defaultBranch, category: project.category || "other", technology: project.technology || [], packageManager: project.packageManager || null, modules: [], warnings: [] } } : null), [validating, setValidating] = useState(false), [fieldError, setFieldError] = useState("");
  const change = (field: keyof Form, value: string) => { dispatch({ type: "change", form: { ...state.form, [field]: value } }); setValidation(current => validationAfterChange(current, field, value)); setFieldError(""); };
  const validate = async () => { setValidating(true); setFieldError(""); try { const result = await post<Inspection>("/projects/validate", { repoPath: state.form.repoPath, defaultBranch: state.form.defaultBranch }); setValidation({ repoPath: state.form.repoPath, defaultBranch: state.form.defaultBranch, result }); } catch (e: any) { setValidation(null); setFieldError(e.message); } finally { setValidating(false); } };
  const save = async (event: React.FormEvent) => { event.preventDefault(); const parsed = parseAllowedCommands(state.form.allowedCommands); if (parsed.error) { setFieldError(parsed.error); return; } if (!validation || validation.repoPath !== state.form.repoPath || validation.defaultBranch !== state.form.defaultBranch) { setFieldError("请先验证当前仓库路径和默认分支"); return; } dispatch({ type: "submit" }); try { const body: Record<string, unknown> = { name: state.form.name.trim(), category: state.form.category || null, allowedCommands: parsed.commands, sensitivePatterns: state.form.sensitivePatterns.split(/\r?\n/).map(item => item.trim()).filter(Boolean) }; if (!project || state.form.repoPath !== project.repoPath) body.repoPath = validation.result.repoPath; if (!project || state.form.defaultBranch !== project.defaultBranch) body.defaultBranch = state.form.defaultBranch.trim(); if (project) await patch(`/projects/${project.id}`, body); else await post("/projects", body); await onSaved(); } catch (e: any) { dispatch({ type: "failure", error: e.message }); } };
  return <div className="modal-backdrop"><div className="modal project-editor" role="dialog" aria-modal="true" aria-labelledby="project-editor-title"><div className="modal-head"><div><h2 id="project-editor-title">{readOnly ? "项目详情" : project ? "编辑项目" : "新建项目"}</h2><p>{readOnly ? "归档项目为只读，历史和知识记录仍会保留" : "验证仓库后才能保存路径和分支配置"}</p></div><IconButton title="关闭" onClick={onClose}><X/></IconButton></div><form onSubmit={save}>
    ...
    <Field label="仓库路径"><input required disabled={readOnly} value={state.form.repoPath} onChange={e => change("repoPath", e.target.value)} placeholder="/absolute/path/to/repository"/></Field><div className="project-branch-row"><Field label="默认分支"><input required disabled={readOnly} value={state.form.defaultBranch} onChange={e => change("defaultBranch", e.target.value)}/></Field>{!readOnly && <button type="button" className="secondary" disabled={validating || !state.form.repoPath.trim() || !state.form.defaultBranch.trim()} onClick={validate}>{validating ? <RefreshCw className="spin" size={16}/> : <CheckCircle2 size={16}/>}验证仓库</button>}</div>
