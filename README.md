...
当前 schema marker 是 `phase-2-quality-coordination-v10`。首次用本版本打开 `phase-2-quality-coordination-v9`、`phase-2-evidence-tree-v8`、`phase-2-evidence-tree-v7`、`phase-2-delivery-quality-v6`、`phase-2-delivery-execution-v5` 或其他旧 live 数据库时，服务先完整备份 SQLite 主文件及现存 WAL/SHM，再创建空的 quality-coordination-v10 数据库。旧历史只存在于备份中；不做 row migration、dual read/write 或 fallback，也没有旧新数据库并行入口。用户已授权旧数据以新跑为准。
