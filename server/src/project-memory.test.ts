...
const artifact={id:"artifact-1",stage:"prd",version:1,content:{productDecisions:[{decision:"用户名必须唯一",rationale:"用于登录",evidence:"SysUser.java"}],facts:["系统用户由后台管理"],risks:["权限配置错误会造成越权"],acceptanceCriteria:["重复用户名创建失败"]}};
...
describe("project memory extraction",()=>{
  it("extracts only approved artifacts with evidence links",()=>{
    ...
    expect(result.every(item=>item.evidence.some(link=>link.artifactId==="artifact-1"&&link.stage==="prd"))).toBe(true);
  });
  ...
  it("deduplicates the same subject across approved artifacts",()=>{
    const second={...artifact,id:"artifact-2",stage:"requirement_review"};
