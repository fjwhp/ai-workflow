...
export async function preflightLocalIntegration(input: Input) {
  ...
  try {
    const repoRoot = (await git(input.targetWorktreePath, ["rev-parse", "--show-toplevel"])).stdout.trim();
    checks.push({ id: "target_repo", label: "目标仓库有效", ok: await realpath(repoRoot) === await realpath(input.targetWorktreePath), detail: repoRoot });
  ...
  return { allowed: checks.length >= 6 && checks.every((entry) => entry.ok), checks, ...plan, evidenceMode, sourceCommit: resolvedSourceCommit, sourceBranch: input.sourceBranch, targetBranch: input.targetBranch, sourceWorktreePath: input.sourceWorktreePath, targetWorktreePath: input.targetWorktreePath };
}
...
export async function executeLocalIntegration(input: Input & { commitMessage: string; commands: VerificationCommand[] }) {
  ...
  const preApplyHead = (await git(input.targetWorktreePath, ["rev-parse", "HEAD"])).stdout.trim();
  try {
    if (!sourceCommit) {
    ...
    try { await git(input.targetWorktreePath, ["cherry-pick", "--abort"]); } catch { /* No active cherry-pick. */ }
    await git(input.targetWorktreePath, ["reset", "--hard", preApplyHead]);
    return { status: "conflict" as const, preflight, sourceCommit, preApplyHead, statusPorcelain: "", conflictFiles, error: String(error?.stderr || error?.message || error), commandResults: [] };
  }
  ...
  for (const item of commands) {
    try {
      ...
      return { status: "test_failed" as const, preflight, sourceCommit, preApplyHead, statusPorcelain, commandResults, error: "本地应用后测试失败" };
    }
  }
  const statusPorcelain = (await git(input.targetWorktreePath, ["status", "--porcelain"])).stdout;
  return { status: "completed" as const, preflight, sourceCommit, preApplyHead, statusPorcelain, commandResults, error: null };
}
