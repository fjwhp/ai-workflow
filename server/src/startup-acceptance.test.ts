...
describe("real server startup acceptance", () => {
  it("backs up deployed automation v4 before creating the quality recovery v8 schema", { timeout: 15_000 }, async () => {
    ...
    try {
      ...
      backup.close();
      await expect(readFile(`${databasePath}.schema-version`, "utf8")).resolves.toBe("phase-2-evidence-tree-v8");
    } finally {
