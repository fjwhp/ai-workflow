...
describe("real server startup acceptance", () => {
  it("backs up deployed automation v4 before creating the terminal resolution v13 schema", { timeout: 15_000 }, async () => {
    ...
    try {
      ...
      backup.close();
      await expect(readFile(`${databasePath}.schema-version`, "utf8")).resolves.toBe("phase-2-terminal-resolution-v13");
    } finally {
