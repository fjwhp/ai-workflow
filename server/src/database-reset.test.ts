...
const currentSchemaVersion = "phase-2-automation-v2";
...
describe("prepareCleanDatabase", () => {
  it("resets a deployed automation v1 database for the v2 marker", async () => {
    const path = await databasePath();
    await writeFile(path, "deployed automation v1 database");
    await writeFile(`${path}.schema-version`, "phase-2-automation-v1");
    ...
    await expect(readFile(result.backupPath!, "utf8")).resolves.toBe("deployed automation v1 database");
  });
