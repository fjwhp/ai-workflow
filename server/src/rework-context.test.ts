...
describe("buildReworkContext",()=>{
  it("normalizes and severity-sorts structured findings",()=>{
    const context=buildReworkContext({approval:{id:"a1",stage:"code_review",target_stage:"coding",actor_type:"ai_gate",created_at:"now",comment:"return"},artifact:{id:"art",content:{findings:[{title:"minor",severity:"S2"},{title:"critical",severity:"S0"}],risks:["risk"],openQuestions:["question"]}}});
  ...
  it("falls back to the approval comment when no artifact exists",()=>{
    const context=buildReworkContext({approval:{id:"a2",stage:"testing",target_stage:"coding",actor_type:"human",created_at:"now",comment:"补充测试"}});
