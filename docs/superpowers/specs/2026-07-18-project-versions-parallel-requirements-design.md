...
在正式进入多项目交付 Phase 2 前，先完成一次可控的真实试运行：同一个本地 Git 项目可以维护多个版本分支，每个版本可以承载多条并行需求。每条需求拥有独立 AI 源分支和临时 worktree，完成后按队列应用到该版本的长期本地 worktree。
...
- 创建版本时，若本地分支不存在，则从用户选择的项目基线分支创建；若已经存在，则校验后登记。
...
- 需求进入编码阶段时，从版本分支当时的 HEAD 创建 `ai/REQ-0001` 和独立临时 worktree。
...
- `branch` 和 `base_branch` 必须是合法本地分支名，并且属于登记项目；
...
.ai-workflow-worktrees/<repo>/
  ...
  requirements/REQ-0001-<run>/ # 需求临时 worktree
```

创建版本：

1. 验证项目仓库、项目默认分支和用户选择的基线分支。
...
进入编码时，从版本分支创建需求源分支和临时 worktree。编码 AI 只操作需求 worktree，不操作版本 worktree 或项目主工作区。
...
4. 创建 `ai/REQ-0001` 和需求临时 worktree；
...
3. 两条需求进入编码后生成 `ai/REQ-0001` 和 `ai/REQ-0002` 两个独立临时 worktree。
