...
2. 在项目的“版本”区域创建新版本，或登记已经存在的本地分支。版本包含名称、版本分支、基线分支、长期 worktree、当前 HEAD 和状态。创建新分支时可使用本地分支或仓库中已存在的精确远端跟踪分支作为基线；系统不会 fetch 或同步远端。登记已有分支不会改写它。若该分支已被外部 worktree 占用，必须明确确认复用。
