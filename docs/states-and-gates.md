...
`phase-2-foundation-v3` 是唯一 live schema marker。v2 或其他旧数据库先备份后 fresh reset；旧历史只在 backup。系统不做 row migration、dual read/write、fallback 或跨 schema 状态兼容。用户已授权旧数据以新跑为准。
