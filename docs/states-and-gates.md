...
`phase-2-quality-coordination-v10` 是唯一 live schema marker。`phase-2-quality-coordination-v9`、`phase-2-evidence-tree-v8`、`phase-2-evidence-tree-v7`、`phase-2-delivery-quality-v6`、`phase-2-delivery-execution-v5` 或其他旧数据库先备份后 fresh reset；旧历史只在 backup。系统不做 row migration、dual read/write、fallback 或跨 schema 状态兼容。用户已授权旧数据以新跑为准。
